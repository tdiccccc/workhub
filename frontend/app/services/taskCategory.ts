...
export const fetchTaskCategory = async (
  ...
  return await $fetch<ApiResponse<TaskCategory>>(
    `/api/projects/${projectId}/task-categories/${taskCategoryId}`,
  );
};

export const createTaskCategory = async (projectId: string | string[], form: TaskCategoryForm) => {
  return await $fetch<ApiResponse<TaskCategory>>(`/api/projects/${projectId}/task-categories`, {
    method: "POST",
    body: form,
  });
